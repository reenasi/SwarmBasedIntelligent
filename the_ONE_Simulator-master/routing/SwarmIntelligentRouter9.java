 ...
 * @author Reena
 */
public class SwarmIntelligentRouter9 implements RoutingDecisionEngine {
    ...
    public SwarmIntelligentRouter9(Settings s) {
        initialNrofCopies = s.getInt(NROF_COPIES_S);
    }

    public SwarmIntelligentRouter9(SwarmIntelligentRouter9 proto) {
    ...
    public void connectionUp(DTNHost thisHost, DTNHost peer) {
        ...
        SwarmIntelligentRouter9 sw = getOtherDecisionEngine(peer);

        //jika historynya ada peer yg ditemui / tdk
        if (!history.containsKey(peer)) {
//            System.out.println("history kosong" +!history.containsKey(peer));
...
//            System.out.println("belum pernah ketemu" + sw.history.get(thisHost));
//            System.out.println("sw.history " + sw.history.put(thisHost, attribute));
        } else {
            //sudah pernah ketemu
...
//                    System.out.println("sudah pernah ketemu node yg sama" + sw.history.get(thisHost) + "sebelum update");
                    ...
                    calcHop = history.get(key).getNrOfHops() + 1;
...
//        Collection<Message> m = peer.getMessageCollection();
    ...
    private SwarmIntelligentRouter9 getOtherDecisionEngine(DTNHost h) {
        MessageRouter otherRouter = h.getRouter();
        assert otherRouter instanceof DecisionEngineRouter : "This router only works "
                + " with other routers of same type";
        return (SwarmIntelligentRouter9) ((DecisionEngineRouter) otherRouter).getDecisionEngine();
    }
    ...
    public RoutingDecisionEngine replicate() {
        return new SwarmIntelligentRouter9(this);
    }
}